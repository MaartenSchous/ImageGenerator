...
namespace _07_Toolset
{
    public class Toolset
    {
        ...
        //v2.7.2 Learned to allign code and make it easyer to read (CTR+K+D in VS)
        ...
        //Required for webhost FTP access:
        ...
        //
        //MySQL
        //All the SQL related functions
        //requires MySql.Data package (NuGet)
        //TODO currently built for my own use, can be made dynamic for generic use

        // Counts the number of entries in the tickets table where the 'columnName' field contains the word 'contains'.
        ...
        public List<String> GetTiers()
        {
            ...
            //prepare query
            string sqlQuery = "SELECT DISTINCT tier FROM tickets;";
        ...
        //returns a list of ticket objects from a specific tier
        public List<TicketParameters> GetAllFromTier(string tier)
        {
            ...
            string sqlQuery = "SELECT * FROM `tickets` WHERE tier LIKE '%" + tier + "%' ORDER BY ImageID DESC;";
        ...
        public StableDiffusionParametersResponse GetNextQueuedEntry(string tier)
        {
            StableDiffusionParametersResponse completeTicket = null;

            StableDiffusionParameters nextTicket = null;

            //Get the oldest queued entry
            //string selectQuery = "SELECT * FROM `tickets` WHERE Status = 'queued' ORDER BY ImageID ASC LIMIT 1;";
            string selectQuery = "SELECT * FROM `tickets` WHERE Status LIKE '%Queued%' AND tier LIKE '%" + tier + "%' ORDER BY ImageID ASC LIMIT 1;";

            //UPDATE Query: Immediately mark the selected entry as 'processing' so if we crash somehow it won't be picked up again
            string updateQuery = "UPDATE `tickets` SET Status = 'processing' WHERE ImageID = @ImageID;";
            ...
            using (MySqlConnection connection = new MySqlConnection(MySQLConnectionString))
            {
                try
                {
                    ...
                    using (MySqlCommand selectCommand = new MySqlCommand(selectQuery, connection))
                    {
                        using (MySqlDataReader reader = selectCommand.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                // Map the first (and only) row to the TicketParameters object
                                nextTicket = new StableDiffusionParameters
                                {
                                    // --- Core Generation Parameters (String) ---
                                    prompt = reader.GetString("Prompt"),
                                    negative_prompt = reader.GetString("NegativePrompt"),
                                    sampler_name = reader.GetString("SamplerName"),

                                    // --- Core Generation Parameters (Int/Double/Long) ---
                                    steps = reader.GetInt32("Steps"),
                                    width = reader.GetInt32("Width"),
                                    height = reader.GetInt32("Height"),
                                    batch_size = reader.GetInt32("BatchSize"),

                                    // Note: CfgScale in DB maps to cfg_scale in API
                                    cfg_scale = reader.GetFloat("CfgScale"),

                                    // Note: Seed is a nullable long in C# and in the API
                                    seed = reader.IsDBNull(reader.GetOrdinal("Seed")) ? -1 : reader.GetInt64("Seed"),

                                    // --- High-Resolution Fix (Hires. fix) Parameters ---
                                    // Note: HiresFix in DB maps to enable_hr in API
                                    enable_hr = reader.GetBoolean("HiresFix"),

                                    // Note: HiresUpscaler in DB maps to hr_upscaler in API
                                    hr_upscaler = reader.GetString("Hiresupscaler"),

                                    // Note: HiresSteps in DB maps to hr_second_pass_steps in API
                                    hr_second_pass_steps = reader.GetInt32("HiresSteps"),

                                    // Note: HiresDenoisingStrength in DB maps to denoising_strength in API (when enable_hr is true)
                                    denoising_strength = reader.GetFloat("HiresDenoisingStrength"),

                                    // Note: HiresUpscaleBy in DB maps to hr_scale in API
                                    hr_scale = reader.GetFloat("HiresUpscaleBy"),
                    ...
                    //Update the entry status if a ticket was found
                    if (nextTicket != null)
                    {
                        using (MySqlCommand updateCommand = new MySqlCommand(updateQuery, connection))
                        {
                            // Use the ImageID we just read to target the row
                            ...
                            if (rowsAffected == 0)
                            {
                                // Should not happen if Step 1 succeeded, but good for safety
                                MessageBox.Show($"Warning: Could not mark ImageID {ImageID} as 'processing'. Check concurrency.", "Queue Lock Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            }
                        }
                    }
                }
                ...
                {
                    MessageBox.Show($"MySQL Error getting queued entry: {ex.Message}", "Database Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                catch (Exception ex)
                {
                    MessageBox.Show($"An unexpected error occurred while getting queued entry: {ex.Message}", "Application Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }

            if (nextTicket != null)
            {
                completeTicket = new StableDiffusionParametersResponse
                {
                    result = nextTicket,
        ...
        // Inserts a new row into the tickets table using the provided parameters. Returns success or failure
        ...
        public bool UpdateStatusTierLocation(TicketParameters row, string MySQLConnectionString)
        {
            //Set the new values WHERE imageID (unique key) matches.
            string updateQuery = @"
            UPDATE tickets 
            SET status = @NewStatus, tier = @NewTier, location = @Location
            WHERE imageID = @ImageID;";
        ...
        public async void LoadWebhostConfigAsync()
        {
            if (File.Exists(MyWebhostConfigLocation))
            {
                try
                {
                    string fileContent = File.ReadAllText(MyWebhostConfigLocation);
            ...
            {
                MessageBox.Show($"Webhost configuration file not found at {MyWebhostConfigLocation}.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
